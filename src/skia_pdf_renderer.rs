...
pub struct SkiaPdfRenderer {
    document: Document,
}

impl SkiaPdfRenderer {
    ...
    fn pixmap_to_image(&self, pixmap: Pixmap) -> Result<RgbaImage> {
        ...
        for (i, pixel) in image.pixels_mut().enumerate() {
            let idx = i * 4;
            // tiny-skia uses premultiplied alpha, but we want straight alpha
            let a = data[idx + 3];
            if a > 0 {
